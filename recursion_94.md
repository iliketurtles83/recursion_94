...
- **Continuous voxel landscape:** the old shader floor is replaced by deterministic stepped rock columns across a 512-unit terrain range. A continuous low central trough fills the lower frame while correlated terraces rise into broad shoulders, giving the flight path depth without exposing a flat baseline or the backdrop beneath it.
...
- **Independent horizon silhouettes:** a fixed 128-item far pool places selected grounded towers between the 384-unit full-detail boundary and the 512-unit terrain horizon. These objects skip expensive architecture-detail passes, keeping the distance populated without increasing foreground clutter.
